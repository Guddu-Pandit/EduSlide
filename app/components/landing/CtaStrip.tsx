import Link from "next/link";

export default function CtaStrip() {
  return (
    <section className="bg-[#0f6e56] px-6 py-16 text-center sm:px-10">
      <h2 className="mb-3 font-display text-[34px] font-bold tracking-tight text-white">
        Ready to reclaim your prep time?
      </h2>
      <p className="mb-7 text-base text-white/70">
        Free for the first 10 decks. No credit card required.
      </p>
      <Link
        href="/signup"
        className="inline-block rounded-xl bg-white px-8 py-3.5 text-[15px] font-semibold text-[#0f6e56] transition-opacity hover:opacity-90"
      >
        Create your free account ↗
      </Link>
    </section>
  );
}
