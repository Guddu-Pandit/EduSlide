...
export default function UploadForm({
  ...
  return (
    ...
    >
      <div className="flex flex-col rounded-xl border border-border-soft bg-surface-1 p-5">
        ...
        {submitting ? (
          ...
          <div className="flex flex-1 flex-col">
            <div
              ...
              className={`flex flex-1 cursor-pointer flex-col items-center justify-center rounded-xl border-[1.5px] border-dashed px-5 py-10 text-center transition-colors ${
                dragOver ? "border-brand bg-brand-tint" : "border-border-mid hover:border-brand hover:bg-brand-tint"
              }`}
            >
              <div className="mb-3.5 flex h-14 w-14 items-center justify-center rounded-full bg-brand-tint">
                <CloudUpload className="h-7 w-7 text-brand" />
              </div>
              <div className="mb-1.5 text-[15px] font-semibold text-text-strong">
                Drop your file here or click to browse
              </div>
              <div className="mb-4 text-[13px] text-text-muted">Max 25 MB</div>
              <div className="flex gap-1.5">
                {(["pdf", "docx", "txt"] as const).map((t) => (
                  <span
                    key={t}
                    className="rounded-md border border-border-soft bg-surface-1 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-text-muted"
                  >
                    {t}
                  </span>
                ))}
      ...
      <div className="flex flex-col rounded-xl border border-border-soft bg-surface-1 p-5">
        ...
        <button
          ...
          className="mt-auto flex w-full items-center justify-center gap-1.5 rounded-lg bg-brand px-4 py-2.5 text-[13px] font-semibold text-white transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40"
        >
          {submitting ? (
            <>
