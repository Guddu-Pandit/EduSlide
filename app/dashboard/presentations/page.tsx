...
function PresentationsContent() {
  ...
  const status = (searchParams.get("status") as PresentationStatus) || undefined;
  const q = searchParams.get("q") || undefined;

  const { data: presentations, loading } = useDashboardQuery(
    (supabase, userId) => getPresentations(supabase, userId, { status, q }),
    [status, q],
  );
