    ...
    <div className="p-6">
      ...
      <div className="rounded-xl border border-admin-border bg-admin-surface">
        <div className="flex items-center justify-between border-b border-admin-border px-4 py-3">
          <div className="text-[13px] font-semibold text-admin-text">Recent Activity</div>
        ...
        {activity.length === 0 ? (
          <div className="px-4 py-10 text-center text-[13px] text-admin-muted">No recent activity</div>
        ) : (
          <div className="divide-y divide-admin-divider">
            {activity.map((item, i) => {
              const { Icon, bg, color } = iconConfig[item.kind];
              return (
                <div key={i} className="flex cursor-pointer items-start gap-3 px-4 py-3 hover:bg-admin-hover">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg" style={{ background: bg, color }}>
                    <Icon className="h-4 w-4" />
                  </div>
                  <div className="flex-1">
                    <div className="text-[13px] font-medium text-admin-text">{item.label}</div>
                    <div className="mt-0.5 text-[11px] text-admin-muted">
                      {new Date(item.at).toLocaleString("en-IN", { day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })}
                    </div>
                  </div>
                  <div className="text-[11px] text-admin-muted">{formatRelativeTime(item.at)}</div>
                </div>
              );
            })}
          </div>
